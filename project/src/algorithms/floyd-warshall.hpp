...
void floydWarshall(const GasGraph& graph, vector< vector<ulong> >& D, vector< vector<ulong> >& R){
  ...
  D.assign(graph.getVertices(), vector<ulong>(graph.getVertices(), infty));
  R.assign(graph.getCities(), vector<ulong>(graph.getCities(), infty)); //El -1 es para no contar el (v, v)
